...
from stsgcn.datasets import H36M_3D_Dataset
from stsgcn.utils import mpjpe_error
...
def visualize(model, device, cfg):
    input_n = cfg["input_n"]
    ...
    for action in actions:
        loader = H36M_3D_Dataset(data_dir, input_n, output_n, skip_rate, body_model_dir, actions=[action], split=2)
        ...
        for cnt,batch in enumerate(loader): 
            ...
            sequences_predict = model(sequences_train).contiguous().view(-1, output_n, len(dim_used))
            ...
            line_anim.save(os.path.join(cfg["log_dir"], cfg["experiment_time"], f'human_viz_{action}_{cnt}.gif'), writer='pillow')
