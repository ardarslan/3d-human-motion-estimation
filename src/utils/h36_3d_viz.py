...
def visualize(input_n, output_n, visualize_from, path, modello, device, n_viz, skip_rate, body_model_dir, actions):
    ...
    for action in actions:
        ...
        for cnt, batch in enumerate(loader):
            ...
            sequences_train = (
                batch[:, 0:input_n, dim_used]
                .view(-1, input_n, len(dim_used) // 3, 3)
                .permute(0, 3, 1, 2)
            )
            sequences_gt = batch[:, input_n : input_n + output_n, :]

            sequences_predict = (
                modello(sequences_train)
                .permute(0, 1, 3, 2)
                .contiguous()
            ...
            loss = mpjpe_error(
                all_joints_seq, sequences_gt
            )  # # both must have format (batch,T,V,C)
            ...
            line_anim = animation.FuncAnimation(
                ...
                update,
                output_n,
                fargs=(data_gt, data_pred, gt_plots, pred_plots, fig, ax),
                interval=70,
                blit=False,
            )
            plt.show()
