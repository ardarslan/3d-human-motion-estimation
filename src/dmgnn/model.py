...
from graphs_layers import *


class GNNGRU(nn.Module):
    
    def __init__(self, n_in_enc, n_hid_enc, n_in_dec, n_hid_dec, fusion_layer, cross_w, **kwargs):
        super().__init__()
        self.model_name = "gnngru"
        #Encode Pos, Vel and Acc, initializing relrec params here try!!!
        #if layout == "h36m":
        V, W, U = 20, 10, 5
        off_diag_joint, off_diag_part, off_diag_body = np.ones([V, V])-np.eye(V, V), np.ones([W, W])-np.eye(W, W), np.ones([U, U])-np.eye(U, U)
        self.relrec_s1 = torch.FloatTensor(np.array(encode_onehot(np.where(off_diag_joint)[1]), dtype=np.float32))
        self.relsend_s1 = torch.FloatTensor(np.array(encode_onehot(np.where(off_diag_joint)[0]), dtype=np.float32))
        self.relrec_s2 = torch.FloatTensor(np.array(encode_onehot(np.where(off_diag_part)[1]), dtype=np.float32))
        self.relsend_s2 = torch.FloatTensor(np.array(encode_onehot(np.where(off_diag_part)[0]), dtype=np.float32))
        self.relrec_s3 = torch.FloatTensor(np.array(encode_onehot(np.where(off_diag_body)[1]), dtype=np.float32))
        self.relsend_s3 = torch.FloatTensor(np.array(encode_onehot(np.where(off_diag_body)[0]), dtype=np.float32))
        
        #Encoder
        self.encoder_pos = Encoder(n_in_enc, fusion_layer, cross_w, **kwargs)
        self.encoder_vel = Encoder(n_in_enc, fusion_layer, cross_w, **kwargs)
        self.encoder_acl = Encoder(n_in_enc, fusion_layer, cross_w, **kwargs)
        
        #Decoder
        self.decoder = Decoder(n_in_dec, n_hid_dec, graph_j, **kwargs)
        self.linear = nn.Linear(n_hid_enc*3, n_hid_dec)
        self.relu = nn.ReLU()

    def forward(self, enc_p, enc_v, enc_a, dec_curr, dec_prev, dec_prev2, t, lamda_p=1):
        #have to include layout
        hidd_p = self.encoder_pos(enc_p, self.relrec_s1, self.relsend_s1, self.relrec_s2, self.relsend_s2, self.relrec_s3, self.relsend_s3, lamda_p)
        hidd_v = self.encoder_vel(enc_v, self.relrec_s1, self.relsend_s1, self.relrec_s2, self.relsend_s2, self.relrec_s3, self.relsend_s3, lamda_p)
        hidd_a = self.encoder_acl(enc_a, self.relrec_s1, self.relsend_s1, self.relrec_s2, self.relsend_s2, self.relrec_s3, self.relsend_s3, lamda_p)
...
class Encoder(nn.Module):
    ...
    def __init__(self, n_in_enc, layout, fusion_layer, cross_w, **kwargs):
        super().__init__()
        #Getting the graphs from joints, parts and body
        self.graph_j = Graph_J(layout)
        self.graph_p = Graph_P(layout)
        self.graph_b = Graph_B(layout)
        A_j = torch.tensor(self.graph_j, dtype=torch.float32, requires_grad=False)
        self.register_buffer('A_j', A_j)
        A_p = torch.tensor(self.graph_p, dtype=torch.float32, requires_grad=False)
        self.register_buffer('A_p', A_p)
        A_b = torch.tensor(self.graph_b, dtype=torch.float32, requires_grad=False)
        self.register_buffer('A_b', A_b)
        
        #setting the kernel size for joints, parts and body
        ...
        #Averaging Joints, Part
        self.s2_init = AverageJoint()
        self.s3_init = AveragePart()
        ...
        if self.fusion_layer == 0:
            ...
            self.j2p_1 = S2S_forward(n_j1=32, n_j2=(800, 256), n_p1=32, n_p2=(800, 256), t_kernel=5, t_stride=(1,2), t_padding=2)
            self.p2j_1 = S2S_backward(n_p1=32, n_p2=(800, 256), n_j1=32, n_j2=(800, 256), t_kernel=5, t_stride=(1,2), t_padding=2)
            self.p2b_1 = S2S_forward(n_p1=32, n_p2=(800, 256), n_b1=32, n_b2=(800, 256), t_kernel=5, t_stride=(1,2), t_padding=2)
            self.b2p_1 = S2S_backward(n_b1=32, n_b2=(800, 256), n_p1=32, n_p2=(800, 256), t_kernel=5, t_stride=(1,2), t_padding=2)
...
class Decoder(nn.Module):
    ...
    def __init__(self, n_in_dec, n_hid_dec, graph_args_j, dropout=0.3, **kwargs):
        super().__init__()
        ...
        self.mask = torch.ones(60).cuda().detach()
        self.zero_idx = torch.tensor([10, 11, 16, 17, 22, 23, 43, 44, 55, 56]).cuda().detach()
